(function () {
  ...
  async function loadModuleHtml(id) {
    ...
    try {
      ...
      if (!html) {
        var res = await fetch('modules/' + encodeURIComponent(id) + '.html?v=20260716-mp6', { credentials: 'same-origin' });
        if (!res.ok) {
  ...
  function prefetchModuleHtml(id) {
    ...
    fetch('modules/' + encodeURIComponent(id) + '.html?v=20260716-mp6', { credentials: 'same-origin' })
