...
- 全局检索模式：未配置百炼密钥时的预期行为，仍可检索项目、任务、周报、训练、标注、文献、数据集、共享文件和知识库。
- 大模型模式：在「OpenAI入口」保存阿里云百炼 API Key 后刷新对话页，角标应变为「大模型模式」。
- 大模型失败会自动回退全局检索，并在气泡下方显示失败原因。
- 本机代理需 `python start_web.py`；否则会尝试远程 `API_PROXY`。
- 全局数据不同步：确认 Pages 已部署含 `config.js` 的版本，并检查对应模块数据是否已写入云端同步键。
...
1. 确认任务卡片显示「云端共享（全员可看）」或详情里已有 `cloudShare`。
2. 若只有本机备份：打开详情点「发布到团队云端」。
3. 其他人点「导出」会优先从云端分片组装 ZIP。
4. 云端桶 404 可忽略：当前默认已走 patents 分片通道，不再强依赖 Storage 桶。
