...
1. 校验归档、`.sha256` 和 `.verified`；
2. 拒绝异常路径、链接、特殊文件和不符合格式的内容；
3. 流式列举并校验四个文件卷，不在 `/tmp` 重复完整解包大卷；
4. 创建随机命名的隔离 Compose 项目和独立数据卷；
5. 恢复 MinIO 与 PostgreSQL；
6. 在隔离副本上依次运行当前数据库迁移和 MinIO 应用身份初始化；
7. 挂载状态、上传和日志卷，启动无宿主机端口的隔离网关；
8. 检查必要数据表、账号、同步数据，以及 MinIO 对象列举和抽样读取；
9. 输出备份年龄和近似恢复耗时，最后删除隔离容器、卷和临时目录。
